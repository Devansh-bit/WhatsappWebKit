...
driver = Initializer.create_driver("D:\PythonProjects\WhatsappMod\chromedriver.exe")
window = Initializer.WebDriver(driver)
meet_driver = Initializer.create_meet_driver("D:\PythonProjects\WhatsappMod\chromedriver.exe")
window.meet_sign_in(meet_driver, "xyznfc@gmail.com")
...
while True:
    ...
    if "https://meet.google.com/" in text:
        window.open_meet(meet_driver, (re.search("(?P<url>https?://[^\s]+)", text).group("url")), class_time=45)
